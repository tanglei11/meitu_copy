...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if ([[view.superview viewController] isKindOfClass:NSClassFromString(@"NearByController")]) {
        if (([view isKindOfClass:NSClassFromString(@"VTContentView")] || [view isKindOfClass:NSClassFromString(@"TapDetectingView")])) {
            self.scrollEnabled = NO;
        }else{
            self.scrollEnabled = YES;
        }
    }
    if ([[view.superview viewController] isKindOfClass:NSClassFromString(@"HomePageController")]) {
        self.scrollEnabled = YES;
    }
    NSLog(@"%@",[view.superview viewController]);
    return view;
}
